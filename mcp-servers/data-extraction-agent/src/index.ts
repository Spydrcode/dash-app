...
class DataExtractionMCPServer {
      ...
      tools: [
        {
          ...
          description:
            "Extract trip data from screenshot using OCR and LLM processing",
          inputSchema: {
            type: "object",
            properties: {
              image_path: {
        ...
        },
        {
          ...
          description:
            "Process raw text through LLM to extract structured trip data",
          inputSchema: {
            type: "object",
            properties: {
              raw_text: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        case "extract_trip_data":
          return await this.extractTripData(args as {
            image_path: string;
            image_base64?: string;
          });

        case "process_text_extraction":
          return await this.processTextExtraction(args as {
            raw_text: string;
            platform?: string;
          });
  ...
  private buildExtractionPrompt(rawText: string, platform?: string): string {
    return `
...
JSON:`;
  ...
  private parseExtractionResponse(response: string): TripData {
    try {
  ...
  private getValidationNotes(data: TripData): string[] {
    const notes: string[] = [];

    if (!data.pickup_location) notes.push("Missing pickup location");
    if (!data.dropoff_location) notes.push("Missing dropoff location");
    if (!data.fare_amount) notes.push("Missing fare amount");
    if (!data.trip_date) notes.push("Missing trip date");
